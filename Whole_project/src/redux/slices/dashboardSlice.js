  ...
  async (_, { rejectWithValue }) => {
    try {
      const [statsResponse, schoolsResponse] = await Promise.all([
        axios.get('/api/dashboard/stats'), // Assuming an endpoint for stats
        axios.get('/api/schools'), // Existing schools endpoint
      ]);
      return {
...
export const { enableRealTime, disableRealTime, updateStats } = dashboardSlice.actions;
