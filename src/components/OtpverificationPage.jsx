...
function OtpVerificationPage() {
  ...
  useEffect(() => {
    ...
    if (resendCooldownTimer > 0) {
      timer = setTimeout(
        () => setResendCooldownTimer(resendCooldownTimer - 1),
        1000,
      );
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.canResetPassword && response.data.resetToken) {
        // Assuming backend sends a resetToken
        navigate(`/reset-password/${response.data.resetToken}`, {
          state: { email: emailForOtp },
        });
      ...
      if (apiError) {
        ...
        if (apiError.lockoutUntil) {
          ...
          setLockoutMessage(
            `Max attempts reached. Account locked for 20 minutes.`,
          );
        }
      }
    }
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      setSuccessMessage(
        response.data.message || `A new OTP has been sent to ${emailForOtp}.`,
      );
      ...
      <div className="lg:w-1/2 w-full flex items-center justify-center text-center md:px-16 px-0 z-0 bg-black">
        ...
        <div className="w-full py-6 z-20">
          ...
          <p className="text-base text-gray-300 mb-6 px-2">
            We've sent an OTP to {emailForOtp || "your email"}. Please enter it
            below.
          </p>
          ...
          >
            ...
            {isLockedOut && lockoutMessage && (
              <p className="text-lg text-red-500 mb-4 font-semibold">
                {lockoutMessage}{" "}
                {lockoutTimer > 0 && ` (${formatTime(lockoutTimer)})`}
              </p>
            )}
            {error && !isLockedOut && (
            ...
            <div className="mt-4 text-center text-sm text-gray-400">
              ...
              <button
                ...
                disabled={
                  resendDisabled || isLockedOut || resendCooldownTimer > 0
                }
              ...
              >
                Click to resend{" "}
                {resendCooldownTimer > 0 && `(${resendCooldownTimer}s)`}
              </button>
            </div>
            {/* ... Back to Sign Up link ... */}
            <div className="mt-8 text-center text-sm text-gray-400">
              <Link
                to={isLockedOut ? "/forgot-password" : "/signup"}
